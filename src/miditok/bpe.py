...
def bpe(tokenizer: Type[MIDITokenizer], *args, **kwargs):
    ...
    class BPE(tokenizer):
        ...
        def bpe(self, tokens_path: Union[Path, PurePath, str], out_dir: Union[Path, PurePath, str], vocab_size: int = None, n_merges: int = None, 
            ...
            """
            assert (vocab_size is not None) or (n_merges is not None) 
            ...
            # Byte Pair Encoding
            duration_tqdm = vocab_size - len(self.vocab) if vocab_size is not None else n_merges
            jj = 0
            pbar = tqdm(total=duration_tqdm, desc='Learning byte pair encoding')
            is_computing_bpe = len(self.vocab) < vocab_size if vocab_size is not None else jj < n_merges
            while is_computing_bpe:
                is_computing_bpe = len(self.vocab) < vocab_size if vocab_size is not None else jj < n_merges
                jj += 1
                pbar.update(1)
                ...
                self.supertoken_frq[to_replace_str] = occurrences[to_replace]
        ...
        def __subfind(in_list: List[int], pattern: List[int]) -> List[int]:
            ...
            XXX : FIXED to avoid overlapping
            """
        ...
        def decompose_bpe(self, tokens: List[int], inplace=False) -> List[int]:
