...
class Vocabulary:
    ...
    def reindex_vocab(self):
        r"""Reindex the vocabulary when the token numbering is messed up.
        (Due to token type removal for example)
        """
        temp_event_to_token = {}
        temp_token_to_event = {}
        temp_token_types_index = {}
        for index, (event, token) in enumerate(self._event_to_token.items()):
            temp_event_to_token[event] = index
            temp_token_to_event[index] = event
            event_type = self.token_type(token)
            if event_type in temp_token_types_index:
                temp_token_types_index[event_type].append(index)
            else:
                temp_token_types_index[event_type] = [index]
                
        self._event_to_token = temp_event_to_token
        self._token_to_event = temp_token_to_event
        self._token_types_indexes = temp_token_types_index
    
    
    def remove_token_type(self, token_type: str):
        r"""Remove all the tokens of a same type.
        
        :param token_type: token type to be removed
        """
        events_to_rm = [ev for ev in self._event_to_token if ev.startswith(token_type)]
        for ev in events_to_rm:
            token_to_pop = self._event_to_token.pop(ev, None)
            self._token_to_event.pop(token_to_pop, None)
        self._token_types_indexes.pop(token_type, None)
        self.reindex_vocab()
